...
export function MusicPlayer({ isOpen, onClose, isDark }: MusicPlayerProps) {
  ...
  useEffect(() => {
    const loadPlaylist = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(`HTTP ${response.status}: Không thể tải playlist`);
        }
        ...
        if (!data.tracks || !Array.isArray(data.tracks)) {
          throw new Error('Định dạng playlist không hợp lệ');
        }
        
        if (data.tracks.length === 0) {
          throw new Error('Playlist trống');
        }
        
        setTracks(data.tracks);
        setPlaylistError(null);
        console.log('Playlist loaded successfully:', data.tracks);
      } catch (error) {
        console.error('Error loading playlist:', error);
        setPlaylistError(error instanceof Error ? error.message : 'Lỗi không xác định');
      }
    };
  ...
  const reloadPlaylist = () => {
    ...
    const loadPlaylist = async () => {
      try {
        const response = await fetch('/music/playlist.json?t=' + Date.now());
        const data = await response.json();
        setTracks(data.tracks || []);
      } catch (error) {
        setPlaylistError(error instanceof Error ? error.message : 'Lỗi tải playlist');
      }
    };
    loadPlaylist();
  };
          ...
          >
            ...
            >
              ...
              <div className={`p-3 rounded-xl mb-4 text-xs ${
                ...
                <p>Current track: {currentTrack?.file || 'None'}</p>
  ...
  <div className={`relative h-2 rounded-full mb-2 ${isDark ? 'bg-slate-700' : 'bg-slate-200'}`}>
    ...
    {/* invisible range overlay để drag/seek */}
    <input
      type="range"
      min={0}
      max={duration || 0}
